...
def x5t_s256_from_der(der: bytes) -> str:
    """RFC 8705 `x5t#S256`: base64url(SHA-256(DER cert)), no padding. This is
    the value embedded in a certificate-bound token's `cnf` claim and compared
    against the client cert presented on each request."""
    import base64
    import hashlib
    return base64.urlsafe_b64encode(hashlib.sha256(der).digest()).rstrip(b"=").decode()


def x5t_s256(cert) -> str:
    from cryptography.hazmat.primitives.serialization import Encoding
    return x5t_s256_from_der(cert.public_bytes(Encoding.DER))
