...
def main():
    ...
    for name, (scopes, resources) in AGENTS.items():
    ...
    print("Get a cert-bound token (mTLS), then call with it:")
    print("  curl --cacert certs/ca.crt --cert certs/billing-agent.crt \\")
    print("       --key certs/billing-agent.key -X POST https://127.0.0.1:5000/v1/token")
    print("Or run the full demo (incl. stolen-token replay):")
