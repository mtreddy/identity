...
SCOPE = "openid profile email resources:read"   # `openid` -> we get an id_token
...
def main():
    ...
    # 8. AUTHORIZATION — call the resource server with the access token
    def api(path):
        req = urllib.request.Request(BASE + path)
        req.add_header("Authorization", "Bearer " + tok["access_token"])
        with urllib.request.urlopen(req) as r:
            return r.status, json.load(r)
    print(f"8. GET /userinfo             -> {api('/userinfo')}")
    print(f"   GET /api/resources        -> {api('/api/resources')}")

    # 9. codes are one-time: replay must fail
    st, _, body = _open("POST", BASE + "/token", {
        "grant_type": "authorization_code", "code": code,
        "redirect_uri": REDIRECT_URI, "client_id": CLIENT_ID,
        "code_verifier": verifier,
    })
    print(f"9. POST /token (replay code) -> {st} {body.strip()}")
