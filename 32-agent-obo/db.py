...
Tables:
  ...
  * users     — the human principals an agent can act **on behalf of**, each with
                its OWN authority (scopes + models). An OBO token is downscoped to
                the intersection of the user's authority and the agent's, so the
                agent can never do more than the user it serves.
...
# --- users (the OBO subjects) -----------------------------------------------

def create_user(user_id, name, allowed_scopes, allowed_models):
    conn = get_connection()
    conn.execute(
        "INSERT INTO users (user_id, name, allowed_scopes, allowed_models)"
        " VALUES (?,?,?,?)",
        (user_id, name, allowed_scopes, allowed_models),
    )
    conn.commit()
    conn.close()
