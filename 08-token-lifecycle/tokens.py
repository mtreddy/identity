...
def issue_token(client_id: int, name: str, scopes: list[str]) -> tuple[str, int, str]:
    """Mint a signed JWT for a client. Returns (token, ttl_seconds, jti).

    Every token carries a unique `jti` (JWT ID). That id is what lets us revoke
    an individual access token before it expires: we put revoked jtis on a
    deny-list and check it on each request (see app.py / db.is_jti_revoked)."""
    now = int(time.time())
    jti = uuid.uuid4().hex
    payload = {
        "iss": JWT_ISS,
    ...
    return jwt.encode(payload, _secret(), algorithm=JWT_ALG), TOKEN_TTL, jti
