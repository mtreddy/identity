...
Tables:
  ...
  * api_keys  — the INSECURE static bearer keys for the /vuln foil only.
...
def init_schema():
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS api_keys (       -- /vuln foil only
...
def create_api_key(client_id: str) -> str:
    """Create a long-lived static key (the anti-pattern); return it once."""
    key = "sk-legacy-" + secrets.token_urlsafe(24)
    conn = get_connection()
    conn.execute("INSERT INTO api_keys (key_hash, client_id) VALUES (?,?)",
                 (_sha256(key), client_id))
    conn.commit()
    conn.close()
    return key


def client_for_api_key(key: str):
    conn = get_connection()
    try:
        row = conn.execute("SELECT client_id FROM api_keys WHERE key_hash = ?",
                           (_sha256(key),)).fetchone()
