"""
app.py — "Sign in with SSO, then read your mailbox".

A concrete authenticate-then-authorize-to-access-a-resource demo: the resource
is a mock MAILBOX. A third-party "MailViewer" app uses OpenID Connect to (a)
authenticate the user and (b) get their consent to the `mail:read` scope, then
reads their inbox from the resource server. The mailbox is seeded fake email in
SQLite — no real mail server needed.
...
world are separate parties. They're kept in clearly-labelled sections:
  ...
  (B) RESOURCE SERVER       /userinfo  /api/mailbox
...
SCOPE_DESCRIPTIONS = {
    ...
    "mail:read": "Read the messages in your mailbox",
}
...
@require_token(scope="mail:read")
def api_mailbox():
    """The protected MAILBOX. Only reachable with an access token that carries
    the `mail:read` scope the user consented to — a token with just `openid`
    (identity) but not `mail:read` gets 403 here."""
    rows = db.get_messages_for_user(int(g.claims["sub"]))
...
def openid_configuration():
    ...
    return jsonify({
        ...
        "scopes_supported": ["openid", "profile", "email", "mail:read"],
...
# `mail:read` is what authorizes reading the user's mailbox.
CLIENT_SCOPE = "openid profile email mail:read"
...
def client_callback():
    ...
    messages = api("/api/mailbox")["messages"]
    ...
    return render_template(
        ...
        id_claims=id_claims, userinfo=userinfo, messages=messages,
    )
