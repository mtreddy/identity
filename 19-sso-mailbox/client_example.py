...
SCOPE = "openid profile email mail:read"   # `openid` -> id_token; mail:read -> inbox
...
def main():
    # The CLIENT creates PKCE + state + nonce.
    ...
    form = {k: _field(html, k) for k in (
        "csrf_token", "client_id", "redirect_uri", "scope", "state", "nonce",
    ...
    print(f"6. POST /token               -> {st} scope={tok.get('scope')!r} "
          f"(id_token present: {'id_token' in tok})")

    # 7. AUTHENTICATION — validate the id_token like a real OIDC client:
    #    discover the provider, fetch JWKS, verify signature + iss/aud/exp/nonce.
    disc = json.load(urllib.request.urlopen(BASE + "/.well-known/openid-configuration"))
    jwks = json.load(urllib.request.urlopen(disc["jwks_uri"]))
    kid = jwt.get_unverified_header(tok["id_token"])["kid"]
    key = jwt.algorithms.RSAAlgorithm.from_jwk(
        json.dumps(next(k for k in jwks["keys"] if k["kid"] == kid)))
    id_claims = tokens.verify_id_token(tok["id_token"], key, CLIENT_ID, nonce=nonce)
    print(f"7. id_token verified (JWKS)  -> sub={id_claims['sub']} "
          f"email={id_claims.get('email')} nonce_ok=True")

    # 8. AUTHORIZATION — read the mailbox with the mail:read access token
    def api(path):
    ...
    st, info = api("/userinfo")
    print(f"8. GET /userinfo             -> {st} {info.get('email')}")
    st, box = api("/api/mailbox")
    print(f"9. GET /api/mailbox          -> {st} ({len(box['messages'])} messages)")
    for m in box["messages"]:
        flag = "•" if m["unread"] else " "
        print(f"     {flag} {m['from_addr']:<28} {m['subject']}")

    # 10. codes are one-time: replay must fail
    st, _, body = _open("POST", BASE + "/token", {
        "grant_type": "authorization_code", "code": code,
        "redirect_uri": REDIRECT_URI, "client_id": CLIENT_ID,
        "code_verifier": verifier,
    })
    print(f"10. POST /token (replay code) -> {st} {body.strip()}")
