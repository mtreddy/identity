...
def main():
    ...
    print("  # 1) exchange the API key for a short-lived JWT")
    print('  TOKEN=$(curl -s -X POST -H "Authorization: Bearer <KEY>" \\')
    print("           http://127.0.0.1:5000/v1/token | python -c "
          "'import sys,json;print(json.load(sys.stdin)[\"access_token\"])')")
    print("  # 2) call the API with the JWT")
    print('  curl -H "Authorization: Bearer $TOKEN" http://127.0.0.1:5000/v1/resources')
